...
#include <thread>
#include <vector>
// Linux 没有在头文件里定义 SO_REUSEPORT_LB，需要自己定义
#ifndef SO_REUSEPORT_LB
#define SO_REUSEPORT_LB 46
#endif
...
int make_server_socket(uint16_t port)
{
    ...
    // 新增：启用内核级负载均衡
    if (setsockopt(fd, SOL_SOCKET, SO_REUSEPORT_LB, &yes, sizeof(yes)) < 0)
    {
        perror("setsockopt SO_REUSEPORT_LB");
        // 如果失败，说明内核太老(<4.19)，就忽略
    }
...
int main()
{
    ...
    std::cout << "Echo server (SO_REUSEPORT_LB) on 8080, threads=" << num_threads << "...\n";
